...
CREATE TABLE tbl_book(
	book_BookID INT PRIMARY KEY IDENTITY(1,1) NOT NULL,
	book_Title VARCHAR(100) NOT NULL,
	book_PublisherName VARCHAR(100) NOT NULL CONSTRAINT fk_publisher_name1 FOREIGN KEY REFERENCES tbl_publisher(publisher_PublisherName)
...
CREATE TABLE tbl_book_loans(
	...
	book_loans_BranchID INT NOT NULL CONSTRAINT fk_branch_id1 FOREIGN KEY REFERENCES tbl_library_branch(library_branch_BranchID)
	ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE tbl_book_copies (
	...
	book_copies_Branchid INT NOT NULL CONSTRAINT fk_branch_id2 FOREIGN KEY REFERENCES tbl_library_branch(library_branch_BranchID)
	ON UPDATE CASCADE ON DELETE CASCADE,
...
INSERT INTO tbl_publisher
	...
	VALUES
	('Daw Books','375 Hudson Street,New York, NY 10014', '211-366-2000'),
...
INSERT INTO tbl_book_loans
	(book_loans_BookID, book_loans_BranchID, book_loans_CardNo, book_loans_DateOut, book_loans_DateDue)
	VALUES
...
AS
SELECT bc.book_copies_Branchid AS [Branch ID], lb.library_branch_branchName AS [Branch Name],
		bc.book_copies_No_of_Copies AS [No Of Copies], b.book_Title AS [Book Title]
...
AS
SET @DueDate = GETDATE()
SELECT branch.library_branch_branchName AS [Branch Name], book.book_Title AS [Book Title], 
		borrower.borrower_BorrowerName AS [Borrower Name],borrower.borrower_BorrowerAddress AS [Borrower Address],
		loans.book_loans_DateOut AS [Date Out], loans.book_loans_DateDue AS [Due Date]
		...
		WHERE loans.book_loans_DateDue = @DueDate AND branch.library_branch_BranchName = @LibraryBranchName 
GO
...
AS
SELECT branch.library_branch_branchName AS [Branch Name], COUNT (loans.book_loans_BranchID) AS [Total Count]
		FROM tbl_book_loans AS loans 
			INNER JOIN tbl_library_branch AS branch ON loans.book_loans_BranchID = branch.library_branch_branchID
...
AS
SELECT borrower.borrower_BorrowerName AS [Name], borrower.borrower_BorrowerAddress AS [Address],
...
CREATE PROC dbo.BooksByAuthorandBranch
(@AuthorName varchar(50) = 'Stephen King', @BranchName varchar(50) = 'Central')
AS
SELECT book.book_Title AS [Book Title], copy.book_copies_No_of_Copies AS [No of copy],
		branch.library_branch_branchName AS [Branch Name]
		FROM tbl_book_authors AS Author
			INNER JOIN tbl_book AS book ON author.book_authors_BookID = book.book_BookID
			INNER JOIN tbl_book_copies AS copy ON book.book_BookID = COPY.book_copies_BookID
			INNER JOIN tbl_library_branch AS branch ON copy.book_copies_Branchid = branch.library_branch_branchID
		WHERE branch.library_branch_branchName = @BranchName AND author.book_authors_AuthorName = @AuthorName
GO
EXEC dbo.BooksByAuthorandBranch
